...
augsynth_tidy <- function(data) {
  ...
  res <- summary(asyn)$att %>% filter(Time == 2007) %>% select(Estimate, Std.Error)
...
```{r}
synth_diagnosands <- declare_diagnosands(select = c("bias", "coverage"))
