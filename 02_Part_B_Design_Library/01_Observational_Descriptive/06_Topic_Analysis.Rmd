...
design <-
  declare_population(
    document = add_level(
      ...
      N_words_per_document = rpois(N, lambda = 150),
...
document1 <- data %>% dplyr::filter(document == "001") %>% pull(word) %>% paste0(collapse = " ")
document2 <- data %>% dplyr::filter(document == "002") %>% pull(word) %>% paste0(collapse = " ")
